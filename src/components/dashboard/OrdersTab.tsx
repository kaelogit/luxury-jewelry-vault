...
export default function OrdersTab() {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center py-24 gap-4 bg-white rounded-3xl border border-gray-100">
      ...
      <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400">Loading order history...</p>
    </div>
  )
  ...
  if (orders.length === 0) return (
    <div className="py-32 text-center space-y-6 bg-white rounded-3xl border border-gray-100">
      ...
      <div className="space-y-2">
        <h4 className="text-xl font-bold text-black uppercase tracking-tight">No orders yet</h4>
        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">When you make a purchase, it will appear here.</p>
      </div>
    </div>
  )
...
function OrderCard({ order }: { order: any }) {
  const supabase = createClient() // FIX: Using the instance factory
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="flex gap-5 items-center">
            ...
            <div className="space-y-1">
              <p className="text-[10px] font-mono font-bold text-gold uppercase">{order.tracking_number}</p>
          ...
          <div className="flex items-center justify-between md:justify-end w-full md:w-auto gap-8 pt-4 md:pt-0 border-t md:border-t-0 border-gray-50">
            <div className="text-left md:text-right">
              <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Price Paid</p>
          ...
          >
            <div className="p-6 md:p-10 border-t border-gray-100 space-y-10">
              ...
              <div className="space-y-4">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Purchased Assets</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {order.items?.map((item: any, idx: number) => (
                    <div key={idx} className="flex items-center gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
                      <img src={item.image} className="w-16 h-20 object-cover rounded-xl border border-gray-50" alt={item.name} />
                      <div className="space-y-0.5">
                        <p className="text-[9px] font-bold text-gold uppercase">{item.category}</p>
              ...
              <div className="space-y-6">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Tracking Status</p>
                <div className="space-y-8 relative before:absolute before:left-[17px] before:top-2 before:bottom-2 before:w-[1px] before:bg-gray-200">
                  {logs.length > 0 ? logs.map((log, i) => (
                    ...
                    <div className="flex items-center gap-3 text-amber-600 bg-white p-5 rounded-2xl border border-amber-100 shadow-sm ml-2">
                       <AlertCircle size={14} />
                       <p className="text-[10px] font-bold uppercase tracking-widest">Processing order details - Updates will appear shortly.</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="pt-8 border-t border-gray-100 flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-[10px] font-bold text-black uppercase tracking-widest">Professionally Verified & Insured Delivery</p>
                </div>
                <p className="text-[9px] font-bold text-gray-300 uppercase">Official Lume Vault Shipment</p>
              </div>
